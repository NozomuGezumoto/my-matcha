...
class AsyncNgrok {
    ...
    /** Exposed for testing. */ async _getIdentifyingUrlSegmentsAsync() {
        ...
        try {
            ...
            debug('getUserAsync failed (network/API), using anonymous for tunnel:', e && (e.message || String(e)));
            user = null;
        }
        ...
        try {
            ...
            const assertNgrok = ()=>{
                if ((0, _NgrokResolver.isNgrokClientError)(error)) {
                    var _error_body_details, _error_body;
                    const body = error == null ? void 0 : (_error_body = error.body) == null ? void 0 : _error_body;
                    throw new _errors.CommandError('NGROK_CONNECT', [
                        body ? body.msg : null,
                        body && (_error_body_details = body.details) != null ? _error_body_details.err : undefined,
                        _chalk().default.gray('Check the Ngrok status page for outages: https://status.ngrok.com/')
                    ].filter(Boolean).join('\n\n'));
                }
                throw new _errors.CommandError('NGROK_CONNECT', (error && typeof error.toString === 'function' ? error.toString() : String(error)) + _chalk().default.gray('\nCheck the Ngrok status page for outages: https://status.ngrok.com/'));
            };
            ...
            if ((0, _NgrokResolver.isNgrokClientError)(error) && error && error.body && error.body.error_code === 103) {
